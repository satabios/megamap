...
void TOFApp::Init(int w, int h)
{
   float frameRate = 30.0;
    vid.frameRate.numerator = frameRate*10000;
    vid.frameRate.denominator = 10000;
    
    uint g = gcd(vid.frameRate.numerator, vid.frameRate.denominator);
    vid.frameRate.numerator = vid.frameRate.numerator/g;
    vid.frameRate.denominator = vid.frameRate.denominator/g;
...
bool TOFApp::connect()
{
   ...
   if (devices.size() > 0) {
    ...
    for(auto &p: profiles)
    {
      ...
      if (p.second == "Calibrated Normal")  // chandra
      //if (p.second == "Calibrated High Power")
         profile_id = p.first;
      
      ConfigurationFile *c = _depthCamera->configFile.getCameraProfile(129);//129 chandra //profile_id
    ...
    if (_depthCamera->setCameraProfile(129)) //129 chandra
...
//    _depthCamera->set("illum_power_percentage", _illum_power);
//    _depthCamera->set("intg_duty_cycle", _intg);
       _depthCamera->setFrameRate(vid.frameRate);
