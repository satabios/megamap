...
static deque< XYZIPointCloudFrame> qFrame; 
...
void *TOFApp::eventLoop(void *p)
{
   ...
   while (!done) {
      ...
      if (!empty) {
         ...
         pthread_mutex_lock(&gmtx);
         XYZIPointCloudFrame *frm = &qFrame.front(); 
         pthread_mutex_unlock(&gmtx);
...
bool TOFApp::connect()
{
   ...
   _depthCamera->registerCallback(DepthCamera::FRAME_XYZI_POINT_CLOUD_FRAME, frameCallback);
...
void TOFApp::update(XYZIPointCloudFrame *frm)
{
}
